...
- **顶部导航栏**（固定）：
  - 品牌标识：AI-CodeNexus
  - 主要导航：最新资讯（下拉菜单包含编程资讯、AI资讯）、提示词、规则、社区资源、微信公众号
  - 管理员入口（隐藏，需输入授权码显示）
- **左侧边栏**（固定）：
  - 工具分类导航（按研发流程排序）
...
- **路由路径**：
  - `/` 或 `/news` - 编程资讯（通过最新资讯下拉菜单访问）
  - `/ai-news` - AI资讯（通过最新资讯下拉菜单访问）
  - `/prompts` - 提示词模块
  - `/rules` - 规则模块
  - `/resources` - 社区资源模块
...
### 3. 最新资讯（通过导航菜单访问）
**功能描述**：通过"最新资讯"下拉菜单访问编程资讯或AI资讯

**访问方式**：
- 点击顶部导航栏的"最新资讯"按钮
- 在下拉菜单中选择"编程资讯"或"AI资讯"
- 编程资讯路由：`/news`
- AI资讯路由：`/ai-news`
...
**数据来源**：
- 存储位置：`data/prompts/prompts.json`
- 数据格式：JSON数组，每个提示词包含名称、描述、分类、标签、URL、内容等

**功能特性**：
- ✅ 分页加载（每页20条，可配置）
- ✅ 分类筛选（按提示词分类）
- ✅ 搜索功能（支持搜索名称、描述、内容）
- ✅ 提示词卡片展示（名称、描述、标签、内容预览）
- ✅ 查看详情和跳转到源链接
- ✅ 内容直接嵌入在JSON中，无需额外文件

**API接口**：
- `GET /api/prompts?category={category}&page=1&page_size=20&search={keyword}`
- `GET /api/prompts/{identifier}` - 获取单个提示词的详细内容
...
```json
{
  ...
  "content": "提示词完整内容（Markdown格式）",
  "copy_count": 0,
  "created_at": "2025-12-03T23:00:00.000000Z",
  "is_featured": false,
...
### 1. 规则列表 (`/rules`)
**功能描述**：展示Cursor Rules和其他AI编程规则集合

**数据来源**：
- 存储位置：`data/rules.json`
- 数据格式：JSON数组，每个规则包含名称、描述、分类、标签、URL、内容等

**功能特性**：
- ✅ 分页加载（每页20条，可配置）
- ✅ 分类筛选（按规则分类，如"开发规则"）
- ✅ 搜索功能（支持搜索名称、描述、内容）
- ✅ 规则卡片展示（名称、描述、标签、内容预览）
- ✅ 完整的规则内容展示
- ✅ 内容直接嵌入在JSON中，无需额外文件
...
```json
{
  ...
  "tags": ["Cursor", "规则"],
  "url": "https://source-url.com",
  "content": "规则完整内容（Markdown格式）",
  "view_count": 0,
  "created_at": "2025-12-03T22:35:53.584884Z",
  "is_featured": false,
...
### 1. 社区资源列表 (`/resources`)
**功能描述**：展示AI编程教程、文章和社区资源

**数据来源**：
- 存储位置：`data/resources.json`
- 数据格式：JSON数组，每个资源包含标题、描述、类型、分类、标签、URL等

**功能特性**：
- ✅ 按分类模块化显示（飞书知识库、技术社区、Cursor资源等）
- ✅ 支持类型筛选（社区、教程、工具等）
- ✅ 支持分类筛选
- ✅ 资源卡片展示（标题、描述、类型、标签）
- ✅ 跳转到源链接
...
```json
{
  ...
  "author": "作者名称",
  "view_count": 0,
  "created_at": "2025-12-03T22:35:53.584916Z",
  "is_featured": true,
...
### 6. 数据自动备份功能
**功能描述**：系统定时自动将 `data/` 和 `config/` 目录的数据提交到 GitHub，作为数据备份

**备份流程**：
1. **检查变更**：检查 `data/` 和 `config/` 目录是否有变更
2. **添加文件**：如果有变更，使用 `git add` 添加这两个目录
3. **提交变更**：使用带时间戳的 commit message 提交变更
4. **推送到远程**：推送到 GitHub 的 master 分支
5. **冲突处理**：如果推送失败，自动拉取最新代码并重新推送

**功能特性**：
- ✅ 定时自动备份（每天 23:00 执行）
- ✅ 只备份数据目录（`data/` 和 `config/`）
- ✅ 自动冲突处理（拉取最新代码后重新推送）
- ✅ 智能检测（没有变更时跳过提交）
- ✅ 详细的日志记录（便于排查问题）
- ✅ 超时保护（Git 命令执行超时保护）
...
**配置说明**：
- **备份时间**：固定为每天 23:00（Asia/Shanghai 时区）
- **备份目录**：`data/` 和 `config/` 目录
- **目标分支**：`master` 分支
- **Commit 信息**：`chore: auto backup data and config - YYYY-MM-DD HH:MM:SS`

**注意事项**：
- 确保生产环境已配置 Git 用户信息（`git config user.name` 和 `git config user.email`）
- 确保生产环境有 GitHub 的推送权限（SSH key 或 Personal Access Token）
- 如果推送失败，会在日志中记录错误信息，不会影响主程序运行

### 7. 日志存档功能
**功能描述**：系统自动将日志保存到文件，特别是定时任务的关键日志

**日志文件结构**：
- `logs/app_{YYYY-MM-DD}.log` - 所有应用日志（INFO 及以上级别）
- `logs/error_{YYYY-MM-DD}.log` - 错误日志（ERROR 及以上级别）
- `logs/scheduler_{YYYY-MM-DD}.log` - 定时任务专用日志（包含关键前缀）

**日志特性**：
- ✅ 按日期自动轮转（每天午夜）
- ✅ 自动压缩旧日志（zip 格式）
- ✅ 自动清理过期日志（应用日志30天，错误和定时任务日志90天）
- ✅ 异步写入（不阻塞主程序）
- ✅ UTF-8 编码（支持中文）
- ✅ 详细的日志格式（时间、级别、位置、消息）
...
**日志目录**：
- 日志文件保存在项目根目录的 `logs/` 目录
- 日志目录已添加到 `.gitignore`，不会提交到 Git

### 8. 每周资讯推荐功能
**功能描述**：自动整理本周新增的资讯（AI资讯和编程资讯）为Markdown格式，适合复制到微信公众号

**触发时机**：
- 当有资讯被采纳并归档时，自动更新本周的Markdown文件
- 支持手动归档和工具相关资讯采纳两种场景

**文件格式**：
- 文件名：`{年份}weekly{周数}.md`，例如 `2025weekly47.md`
- 保存位置：`data/weekly/` 目录
- 文件格式：Markdown格式，适合微信公众号
...
**功能特性**：
- ✅ 自动计算当前周数（ISO 8601标准）
- ✅ 自动筛选本周新增的资讯（基于 `archived_at` 字段）
- ✅ 文件不存在时自动创建，存在时自动更新
- ✅ 适合微信公众号格式（纯文本链接，无Markdown链接）
- ✅ 自动备份（随 `data/` 目录每日备份到GitHub）

**配置说明**：
- **文件命名**：使用ISO 8601周数标准（周一为一周开始）
- **时间范围**：自动计算本周的周一至周日
- **筛选条件**：只包含本周归档的资讯（`archived_at` 在本周范围内）

**注意事项**：
- 周报文件保存在 `data/weekly/` 目录，会随数据备份一起提交到GitHub
- 文件已添加到 `.gitignore`，但目录结构会保留（通过 `.gitkeep` 文件）
- 如果资讯没有 `archived_at` 字段，不会被包含在周报中
...
#### 获取提示词列表
```
GET /api/prompts?category={category}&page={page}&page_size={page_size}&search={keyword}
```

**参数说明**：
- `category`（可选）：提示词分类，如"编程"、"工具"等
- `page`（可选）：页码，默认1
- `page_size`（可选）：每页数量，默认20，最大100
- `search`（可选）：搜索关键词，支持搜索名称、描述、内容
...
#### 获取规则列表
```
GET /api/rules?category={category}&page={page}&page_size={page_size}&search={keyword}
```

**参数说明**：
- `category`（可选）：规则分类，如"开发规则"等
- `page`（可选）：页码，默认1
- `page_size`（可选）：每页数量，默认20，最大100
- `search`（可选）：搜索关键词，支持搜索名称、描述、内容
...
#### 获取社区资源列表
```
GET /api/resources?type={type}&category={category}&page={page}&page_size={page_size}
```

**参数说明**：
- `type`（可选）：资源类型，如"社区"、"教程"、"工具"等
- `category`（可选）：资源分类，如"飞书知识库"、"技术社区"、"Cursor资源"等
- `page`（可选）：页码，默认1
- `page_size`（可选）：每页数量，默认20，最大100
...
└── weekly/                   # 每周资讯周报
    ├── 2025weekly48.md      # 2025年第48周周报
    └── 2025weekly49.md      # 2025年第49周周报
```
...
```json
{
  "pages": {
    ...
    "news": {
      "id": "page-news",
      "title": "编程资讯",
      "description": "最新技术文章和资讯"
    },
    "ai-news": {
      "id": "page-ai-news",
      "title": "AI资讯",
      "description": "AI领域最新动态和技术文章"
    },
    "prompts": {
      "id": "page-prompts",
      "title": "提示词",
      "description": "精选AI编程提示词，提升开发效率"
    },
    "rules": {
      "id": "page-rules",
      "title": "规则",
      "description": "Cursor Rules和其他AI编程规则"
    },
    "resources": {
      "id": "page-resources",
      "title": "社区资源",
      "description": "AI编程教程、文章和社区资源"
    },
    "recent": {
      "id": "page-recent",
      "title": "最新资讯",
      "description": "编程资讯和AI资讯的最新文章，按时间排序"
    }
  },
  "categories": {
    "tools": {...},
